...
describe("PortfolioShowcase", () => {
  ...
  it("renders the portfolio section with the Kanban project entry", () => {
    render(<PortfolioShowcase profile={profile} />);
    ...
    expect(screen.getAllByText(/kanban studio/i)[0]).toBeInTheDocument();
    expect(
      screen.getAllByRole("link", { name: /view source/i })[0],
    ).toHaveAttribute("href", "https://github.com/clydedumpa/Personcal_projects");
    expect(
      screen.getAllByRole("link", { name: /open live app/i })[0],
    ).toHaveAttribute("href", "https://kanban-studio-production.up.railway.app");
  });
