...
const kanbanStudioUrl =
  process.env.NEXT_PUBLIC_KANBAN_STUDIO_URL?.trim() ||
  "https://kanban-studio-production.up.railway.app";
...
export const profile: ProfileSiteModel = {
  ...
  portfolio: [
    ...
    },
    {
      ...
      format: "Static devotional page",
      status: "Portfolio-ready",
      summary:
        "A polished single-page devotion guide that combines the Sacred Heart booklet content with an interactive Friday checklist and nine-month First Friday tracker.",
      highlights: [
        "Merged booklet prayer content into a standalone responsive HTML page",
        "Built local progress tracking for Friday tasks and nine First Friday commitments",
        "Verified desktop and mobile rendering with Playwright and accessibility checks",
      ],
      links: [
        {
          href: "/sacred-heart-devotion/index.html",
          label: "Open page",
        },
      ],
      note:
        "Served from the portfolio app as a static public page.",
    },
    {
      title: "Kanban Studio",
      format: "Full-stack build",
      status: kanbanStudioUrl
        ? "Live multi-user MVP"
        : "Deployment-ready multi-user MVP",
      summary:
        "A Next.js and FastAPI Kanban workflow app with SQLite persistence, local user registration, drag-and-drop board management, and an AI assistant that can safely propose board updates.",
      highlights: [
        "Implemented local account creation, per-user board storage, and board-shape upgrades in SQLite",
        "Built persistent card moves, column edits, and empty-column drag-and-drop behavior with automated regression coverage",
        "Added an OpenRouter-backed assistant flow that validates AI board changes on the backend before saving",
      ],
      links: kanbanStudioUrl
        ? [
            {
              href: kanbanStudioUrl,
              label: "Open live app",
            },
          ]
        : undefined,
      note:
        kanbanStudioUrl
          ? "Hosted as a standalone app and linked here through NEXT_PUBLIC_KANBAN_STUDIO_URL."
          : "Ready to publish as a standalone app. Set NEXT_PUBLIC_KANBAN_STUDIO_URL in the portfolio deployment once the live Kanban URL exists.",
    },
  ],
  certifications: [
    "ITIL V3",
